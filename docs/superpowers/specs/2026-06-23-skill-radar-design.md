...
2. **Inventory Scanner** — Enumerates *available* capabilities to form the coverage denominator: `~/.claude/skills` + project `.claude/skills` (parse SKILL.md frontmatter: name / description / declared triggers), `.claude/commands`, `.claude/agents`, and `settings.json` `mcpServers`.
...
- Inventory roots: `~/.claude/skills`, project `.claude/skills`, `.claude/commands`, `.claude/agents`, `settings.json` `mcpServers`.
